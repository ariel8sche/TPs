...
a)  ¿Cuántos niveles de privilegio podemos definir en las estructuras de
    paginación? dos niveles, usuario y de kernel
...
b)  ¿Cómo se traduce una dirección lógica en una dirección física? ¿Cómo
    ...
    proceso en pseudocódigo. esta explicado en la practica, haciendo un resumen agarramos la parte alta de cr3 lo extendemos 12 bits(ya que cr3 solo guarda los mas singificativos) y la usamos como base, utilizamos la parte alta de la direccion virtual como indice(multiplicado por el tamaña de dt_entry), en esa direccion va a estar la direccion base de la tabal de paginas, le sacamos los atributos, la extendemos y volvemos a repetir este proceso, llegando asi a la direccion fisica   

c)  ¿Cuál es el efecto de los siguientes atributos en las entradas de la
    tabla de página?

  - `D` se pone en 1 automaticamente si fue escrita 
  - `A`si fue accedida (para metodo de remplazo de pagina)
  - `PCD`1 descativa el cache
  - `PWT` define si es write-through (1) o write-back (0) en cache.
  - `U/S`user/kernel
  - `R/W` read/write
  - `P` present
...
f)  Completen las entradas referentes a MMU de `defines.h` y comprendan
  ...
  - `VIRT_PAGE_DIR(X)`: Dada `X` una dirección virtual calcula el índice dentro del directorio de páginas de la PDE asociada. #define VIRT_PAGE_DIR(X)      (((uint32_t)(X) >> 22) & 0x3FF)

  - `VIRT_PAGE_TABLE(X)`: Dada `X` una dirección virtual calcula el índice dentro de la tabla de páginas de la PTE asociada. #define VIRT_PAGE_TABLE(X)    (((uint32_t)(X) >> 12) & 0x3FF)

  - `VIRT_PAGE_OFFSET(X)`: Dada `X` una dirección devuelve el offset dentro de su página. #define VIRT_PAGE_OFFSET(X)   ((uint32_t)(X) & 0xFFF)

  - `CR3_TO_PAGE_DIR(X)`: Obtiene la dirección física del directorio donde `X` es el contenido del registro `CR3`. #define CR3_TO_PAGE_DIR(X)    ((pd_entry_t *)((uint32_t)(X) & 0xFFFFF000))

  - `MMU_ENTRY_PADDR(X)`: Obtiene la dirección física correspondiente, donde `X` es el campo address de 20 bits en una entrada de la tabla de páginas o del page directory #define MMU_ENTRY_PADDR(X)    ((paddr_t)(X) << 12)
...
g)  ¿Qué es el buffer auxiliar de traducción (*translation lookaside
    ...
    es como una espcie de chache para la traducciones, pq no tener que etsra haciendo el proceso muchas veces 
